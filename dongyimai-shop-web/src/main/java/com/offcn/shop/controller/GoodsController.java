...
public class GoodsController {
	...
	public PageResult search(@RequestBody TbGoods goods, int page, int rows  ){
		...
		return goodsService.findPageshop(goods, page, rows);
	}

	@RequestMapping("/deleMarket")
	public Result deleMarket(Long id,String isMarketable){
		try {
			goodsService.deleMarket(id,isMarketable);
			if (isMarketable.equals("1")){
			return new Result(true,"已申请下架");}
			return new Result(true,"已申请上架");
		} catch (Exception e) {
			e.printStackTrace();
			if (isMarketable.equals("1")){
			return new Result(false,"申请下架失败");}
			return new Result(false,"申请上架失败");
		}
	}
}
